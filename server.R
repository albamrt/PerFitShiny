...
, duration = 15)
  ...
  Flagged.all <- reactive(lapply(Indexs(), function(x) {
    flagged.resp(x, UDlvl = cutoff.value(), ord = TRUE)
  }
                                 ))
  ...
  output$ProfilePlot_Lucky <- renderPlotly({
    plotProfile(data_aux4(), caseRow = as.numeric(input$SelectIndividual), main = "Creative pattern")%>%
      add_trace(x = c(1:3), y = Observed_thirds_relative(), type = "scatter", mode = "lines + markers",
                color = I("gray"), inherit = FALSE, showlegend = FALSE)
  })
  ...
  output$ProfilePlot_Careless <- renderPlotly({
    plotProfile(data_aux5(), caseRow = as.numeric(input$SelectIndividual), main = "Creative pattern")%>%
      add_trace(x = c(1:3), y = Observed_thirds_relative(), type = "scatter", mode = "lines + markers",
                color = I("gray"), inherit = FALSE, showlegend = FALSE)
  })
