...
profiles <- function(flagged.dataframe){
  ...
  nindexs <- ncol(flagged.dataframe) - 2
  profiles$flags <- apply(profiles[,1:nindexs], 1, sum)
  ...
  ind.hline <- cumsum(c(1, tab$Freq))
  ind <- ind.hline[-length(ind.hline)]
...
patterns <- function(data, caseRow, responseOptions) {
  ...
  esp_imputation <- round(Nimputation*p)
  simLucky <- if(esp_imputation + nitems -  Nimputation >= ncorrect & esp_imputation < ncorrect){
    SL = 0
    while(sum(SL) != ncorrect){
      SL <- c(rep(1, times = ncorrect - esp_imputation),
              rep(0, times = nitems - Nimputation - (ncorrect - esp_imputation)),
